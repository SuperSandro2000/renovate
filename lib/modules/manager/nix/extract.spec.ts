...
describe('modules/manager/nix/extract', () => {
  ...
  beforeEach(() => {
    packageFile = 'flake.lock';
  });

  it('returns null when no inputs', async () => {
    expect(await extractPackageFile(flake1Lock, packageFile)).toBeNull();
  });

  it('returns nixpkgs input', async () => {
    expect((await extractPackageFile(flake2Lock, packageFile))?.deps).toEqual([
      {
        depName: 'nixpkgs',
  ...
  it('includes nixpkgs with no explicit ref', async () => {
    expect((await extractPackageFile(flake3Lock, packageFile))?.deps).toMatchObject([
      {
  ...
  it('includes patchelf from HEAD', async () => {
    expect((await extractPackageFile(flake4Lock, packageFile))?.deps).toMatchObject([
      {
  ...
  it('includes ijq from sourcehut without a flake', async () => {
    expect((await extractPackageFile(flake5Lock, packageFile))?.deps).toMatchObject([
      {
  ...
  it('includes home-manager from gitlab', async () => {
    expect((await extractPackageFile(flake6Lock, packageFile))?.deps).toMatchObject([
      {
